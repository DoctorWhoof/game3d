...
'		canvas.Alpha = 0.1
		If asteroid Then DrawWireframe( canvas, Camera, asteroid, Width/2.0, Height/2.0 )
	End
	...
	Method Draw3DLine( canvas:Canvas, camera:Camera, v0:Vec3f, v1:Vec3f, offsetX:Float=0, offsetY:Float=0  )		
		...
		If camera.Viewport.Contains( s0 ) Or camera.Viewport.Contains( s1 )
			canvas.DrawLine( s0.X-offsetX, -s0.Y+offsetY, s1.X-offsetX, -s1.Y+offsetY )
		End
	End
	
	
	Method DrawWireframe( canvas:Canvas, camera:Camera, model:Model, offsetX:Float=0, offsetY:Float=0, push:Float = 0.1 )
...
'			If ray0
'				Echo( "ray0" )
'				Local s0 := camera.ProjectToViewport( ray0.point )
'				canvas.DrawCircle( s0.X - offsetX, -s0.Y + offsetY, 5 )
'			End
'			If ray0 Then Echo ("hit")
'			If ray1 Then Echo ("hit")
'			If ray2 Then Echo ("hit")
...
'			If dot0 > 0 And dot1 > 0 And dot2 > 0
			If( ray0 = Null ) And ( ray1 = Null )
				Draw3DLine( canvas, camera, v0, v1, offsetX, offsetY )
			End
			If( ray1 = Null ) And ( ray2 = Null )
				Draw3DLine( canvas, camera, v1, v2, offsetX, offsetY )
			End
			If( ray2 = Null ) And ( ray0 = Null )
'				Draw3DLine( canvas, camera, v2, v0, offsetX, offsetY )
			End
'			End
		Next
	End
